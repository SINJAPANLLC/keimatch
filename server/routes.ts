...
- 余計な説明や前置きは不要です。データのみ返してください`,
  ...
  "driverWork": "ドライバー作業（以下から選択: 手積み手降ろし, 台車使用, エレベーター有, 階段搬入, 設置作業あり, その他）",
...
2. 不足している情報があれば【1つだけ】質問して確認する（複数の質問を一度にしない）
...
▼ 発着地の識別:
- 「発」「積地」「積込」「積み」「出発地」「Pick」「P」= departureArea/departureAddress
- 「着」「卸地」「卸先」「降ろし」「おろし」「納品先」「Del」「D」「配達先」= arrivalArea/arrivalAddress
- 「A→B」「A～B」「A⇒B」→ A=発地, B=着地
- 「〜経由」「経由地あり」→ description に記載

▼ 車種の解析:
- 「軽バン」「バン」「ライトバン」→ vehicleType: "軽バン"
- 「軽トラ」「軽トラック」「軽トラ幌」→ vehicleType: "軽トラック"
- 「冷凍」「レイトウ」「冷凍車」「冷凍バン」→ vehicleType: "軽冷凍車"
- 「冷蔵」「冷蔵車」→ vehicleType: "軽冷蔵車"
- 「軽ワゴン」「ワゴン」→ vehicleType: "軽ワゴン"
- 「バイク」「原付」「バイク便」→ vehicleType: "バイク便"
- 車種が不明な場合は vehicleType: "軽バン"（軽貨物案件のデフォルト）

▼ 車体タイプの解析:
- 「ハイルーフ」「ハイルーフバン」→ bodyType: "ハイルーフ"
- 「幌」「幌車」「ホロ」→ bodyType: "幌車"
- 「箱」「ボックス」「標準」→ bodyType: "標準ボディ"
- 「PG」「パワゲ」「パワーゲート」→ bodyType: "パワーゲート付き"
- 「冷蔵仕様」→ bodyType: "冷蔵仕様"
- 「冷凍仕様」→ bodyType: "冷凍仕様"

▼ ドライバー作業:
- 「手積み」「手降ろし」「手積手降」「手荷役」→ driverWork: "手積み手降ろし"
- 「台車」「台車使用」→ driverWork: "台車使用"
- 「エレベーター」「EV」→ driverWork: "エレベーター有"
- 「階段」「階段あり」→ driverWork: "階段搬入"
- 「設置」「組立」「設置作業」→ driverWork: "設置作業あり"
- 「車上渡し」「引き渡しのみ」→ driverWork: ""（設定なし）
- 「フォーク」「フォークリフト」→ driverWork: "その他"（description に「フォーク使用」を記載）

▼ 荷姿:
- 「パレ」「パレット」→ loadingMethod: "パレット"
- 「バラ」「バラ積み」→ loadingMethod: "バラ積み"
- 「ダン」「段ボール」「ケース」→ loadingMethod: "段ボール"
- 「フレコン」「フレキシブルコンテナ」→ loadingMethod: "フレコン"

▼ 高速・料金:
- 「高速込」「高速込み」「込」→ highwayFee: "込み"
- 「高速別」「高速別途」「別途」→ highwayFee: "別途"
- 「高速なし」「下道のみ」→ highwayFee: "高速代なし"

▼ 温度管理:
- 「常温」→ temperatureControl: "常温"
- 「冷蔵」「チルド」「0〜10℃」→ temperatureControl: "冷蔵（0〜10℃）"
- 「冷凍」「フローズン」「-18℃」→ temperatureControl: "冷凍（-18℃以下）"
- 「定温」「一定温度」→ temperatureControl: "定温"

▼ 輸送形態（定期/スポット）の自動判定（重要）:
定期 → transportType: "定期":
- 「毎日」「毎週」「定期」「ルート配送」「固定」「月〜金」「週〇日」「週〇回」「月〇回」「継続」「業務委託」「委託案件」「定期便」「ルート」
スポット → transportType: "スポット":
- 「スポット」「単発」「今回のみ」「急ぎ」「至急」「臨時」「イレギュラー」「1回のみ」
- 日付が1日分しか指定されていない場合は「スポット」とする
- どちらか明確でない場合は「スポット」をデフォルトにすること

▼ 積合:
- 「積合可」「混載可」「合積み可」→ consolidation: "可能"
- 「貸切」「積合不可」「専用」→ consolidation: "不可"

▼ 緊急度:
- 「至急」「急ぎ」「本日中」「今日中」「ASAP」→ urgency: "至急"

▼ 引越し案件:
- 「引越」「引っ越し」「単身引越」「引越し」「家財」「単身パック」→ movingJob: "引っ越し案件"

▼ よく使われる荷種キーワード（cargoType）:
- 「食品」「弁当」「お弁当」「惣菜」「デリバリー」→ cargoType: "食品"
- 「宅配」「ヤマト」「佐川」「日通」「EC配送」「Amazon」「楽天」→ cargoType: "宅配"
- 「医療」「医薬品」「薬」「検体」→ cargoType: "医療機器・医薬品"
- 「家電」「精密機器」「PC」→ cargoType: "家電・精密機器"
- 「衣類」「アパレル」「衣料」→ cargoType: "衣類・アパレル"
- 「建材」「資材」→ cargoType: "建材・資材"
- 「書類」「書類便」「メール便」→ cargoType: "書類・印刷物"
- 「花」「生花」→ cargoType: "生花"

===== 地名→都道府県の変換（必ず適用すること） =====
主要都市・地域名から都道府県を正確に推測すること:
...
栃木: 宇都宮, 小山, 那須
群馬: 前橋, 高崎, 太田
埼玉: 熊谷, 大宮, 浦和, さいたま市, 川越, 所沢, 越谷, 春日部, 行田, 上尾, 桶川
千葉: 千葉市, 船橋, 柏, 松戸, 市川, 習志野, 浦安, 市原, 成田, 香取, 木更津
東京: 新宿, 渋谷, 品川, 港区, 江東区, 江戸川区, 葛飾区, 足立区, 墨田区, 荒川区, 北区, 板橋区, 練馬区, 豊島区, 文京区, 千代田区, 中央区, 台東区, 目黒区, 世田谷区, 杉並区, 中野区, 新宿区, 八王子, 立川, 町田, 武蔵野市, 三鷹市, 府中市, 調布市, 多摩, 日野市, 小平, 国分寺, 小金井, 西東京, 昭島, 青梅
神奈川: 横浜市, 川崎市, 相模原, 藤沢, 平塚, 小田原, 厚木, 茅ヶ崎, 中原区, 港北区, 都筑区, 鶴見区, 川崎区, 横須賀, 海老名, 大和市
新潟: 新潟市, 長岡, 上越
富山: 富山市, 高岡
石川: 金沢, 七尾
福井: 福井市, 越前
...
岐阜: 岐阜市, 大垣, 多治見
静岡: 静岡市, 浜松, 沼津, 富士市
愛知: 名古屋, 豊田, 岡崎, 豊橋, 一宮, 春日井, 刈谷
三重: 津市, 四日市, 伊勢
滋賀: 大津, 草津, 彦根
京都: 京都市, 宇治, 舞鶴
大阪: 大阪市, 堺市, 枚方, 東大阪, 豊中, 吹田, 高槻, 茨木, 守口, 八尾, 松原, 寝屋川
兵庫: 神戸市, 姫路, 尼崎, 西宮, 明石, 加古川, 宝塚
奈良: 奈良市, 橿原
和歌山: 和歌山市, 田辺
鳥取: 鳥取市, 米子
島根: 松江, 出雲
...
愛媛: 松山, 今治, 新居浜
高知: 高知市, 四万十
福岡: 福岡市, 北九州, 久留米, 飯塚, 春日, 大野城, 太宰府, 筑紫野
佐賀: 佐賀市, 唐津
長崎: 長崎市, 佐世保
熊本: 熊本市, 八代
大分: 大分市, 別府, 中津
...
===== 日付・時間の解析ルール =====
- 「3/5」「3月5日」「3.5」→ desiredDate: "${new Date().getFullYear()}/03/05"
- 「24積み25」→ desiredDate: 当月24日, arrivalDate: 当月25日
- 「朝イチ」「朝当日」→ departureTime: "午前中"
- 「8時」「8:00」「AM8」「8じ」→ departureTime: "8:00"
- 「8:00〜9:00」→ departureTime: "8:00"（開始時間のみ採用）
- 「12:00指定」「12時着」→ arrivalTime: "12:00"
- 「午後指定」「昼以降」→ departureTime: "午後"
- 「夕方」「17時以降」→ departureTime: "夕方以降"
- 「当日」「即日」→ arrivalDate = desiredDate
- 「翌日」「翌日着」→ arrivalDate = desiredDate + 1日
- 時間の選択肢: 「指定なし, 午前中, 午後, 夕方以降, 終日可」または「0:00〜24:00の1時間刻み」から最も近いものを選ぶ

===== 金額の解析ルール =====
- 2〜3桁の数字のみ → 千円単位として解釈（15→15000, 20→20000, 25→25000, 100→100000）
- 「1.5万」「1万5千」→ 15000
- 「¥20,000」「20000円」→ そのまま20000
- 「要相談」「応相談」「応談」→ price: "要相談"
- 「〜15」「〜20」等の範囲表記 → 上限値を採用（または priceSuggestion に min/max を設定）

===== 運賃相場の目安 =====
- 近距離（同一県内・隣県 〜50km）: 軽バン 7,000〜12,000円, 軽トラ 8,000〜15,000円
- 中距離（50〜150km）: 軽バン 12,000〜22,000円, 軽トラ 15,000〜28,000円
- 長距離（150〜300km）: 軽バン 22,000〜38,000円, 軽トラ 28,000〜45,000円
- 超長距離（300km以上）: 軽バン 38,000〜60,000円, 軽トラ 45,000〜70,000円
- 冷凍・冷蔵は上記の1.3〜1.5倍
- 引越し（単身）: 15,000〜35,000円
- 深夜・早朝（22時〜6時）: 通常運賃の1.2〜1.3倍
- 高速代は別途計上（込みの場合はルート距離で概算）

===== 不足情報の確認ルール =====
情報が不足している場合は、次の優先順位で【1つだけ】質問してください:
1. 発地（departureArea/departureAddress）が不明な場合
2. 着地（arrivalArea/arrivalAddress）が不明な場合
3. 日付（desiredDate）が不明でスポット案件の場合
4. 運賃（price）が不明な場合（「運賃はいくらをご希望ですか？相場もお伝えできます」）
複数の情報が不足していても、1回の返答で聞くのは1つだけにすること。

===== titleの自動生成 =====
titleは必ず以下の形式で自動生成すること:
- 「{departureArea}→{arrivalArea} {cargoType} {vehicleType}」
- 荷種不明: 「{departureArea}→{arrivalArea} {vehicleType}」
- 発着地のみ: 「{departureArea}→{arrivalArea} 軽貨物」

===== 応答のJSON形式 =====
{
  "message": "ユーザーへの返答（自然な日本語のみ。JSONデータ・フィールド名・配列・オブジェクト構文を絶対に含めないこと）",
  "extractedFields": {抽出できたフィールドのみのオブジェクト（抽出できなかったフィールドは含めない）},
  "items": [複数案件の場合は各案件のオブジェクト配列、1件または追加なしの場合は空配列],
  "priceSuggestion": { "min": 最低額数字, "max": 最高額数字, "reason": "根拠" } または null,
  "status": "extracting" | "confirming" | "ready" | "chatting"
}

【最重要ルール】messageにJSONを絶対に入れないこと:
- messageには「"items":」「"departureArea":」等のJSON構文を絶対に含めない
- データはすべてitems/extractedFieldsに入れること
- 良い例: 「荷物情報を読み取りました。フォームに反映しています。発地・着地・日程を確認してください。」
- 悪い例: 「以下の情報を検出: {"departureArea":"神奈川"...」← 絶対NG

statusの意味:
- "extracting": 情報不足で追加入力を求めている
- "confirming": 主要情報は揃った、確認中
- "ready": 登録準備完了
- "chatting": 質問・相談への返答中
...
- messageは丁寧だが堅すぎない日本語で
- 入力テキストに明記されていない情報は推測して設定しないこと（特にdriverWork・loadingMethod・consolidation・temperatureControl）
- 大量データが来た場合、要約をmessageに書き、データはitemsに格納すること${chatFewShotSection}`;
...
重要: 現在の日付は${new Date().toLocaleDateString("ja-JP", { year: "numeric", month: "long", day: "numeric" })}です。日付が年を省略している場合は必ず${new Date().getFullYear()}年として扱ってください。
...
解析ルール（厳守）:
車種: 「バン」「軽バン」→"軽バン", 「軽トラ」→"軽トラック", 「冷凍」「レイトウ」→"軽冷凍車", 「冷蔵」→"軽冷蔵車", 「ワゴン」→"軽ワゴン", 「バイク便」→"バイク便", 不明→"軽バン"
車体: 「ハイルーフ」→"ハイルーフ", 「幌」→"幌車", 「箱」→"標準ボディ", 「PG」「パワゲ」→"パワーゲート付き"
作業: 「手積手降」→"手積み手降ろし", 「台車」→"台車使用", 「エレベーター」「EV」→"エレベーター有", 「階段」→"階段搬入", 「設置」→"設置作業あり"
高速: 「高速込」→"込み", 「高速別」→"別途", 「下道のみ」→"高速代なし"
積合: 「貸切」「専用」→"不可", 「積合可」「混載可」→"可能"
輸送形態: 「毎日」「ルート」「定期」「業務委託」「月〜金」→"定期", それ以外→"スポット"
温度: 「常温」→"常温", 「冷蔵」「チルド」→"冷蔵（0〜10℃）", 「冷凍」→"冷凍（-18℃以下）", 「定温」→"定温"
地名→都道府県: 横浜市・川崎市・相模原→神奈川, 熊谷・大宮・浦和・所沢・春日部→埼玉, 千葉市・船橋・柏・成田・香取→千葉, 江東区・品川・渋谷・新宿・足立区・墨田区・八王子・町田→東京, 名古屋・豊田・岡崎→愛知, 大阪市・堺市・枚方→大阪, 神戸市・姫路→兵庫, 京都市→京都, 福岡市・北九州→福岡, 仙台→宮城, 札幌→北海道, 広島市→広島
日付: 「3/5」→"${new Date().getFullYear()}/03/05", 「当日」→arrivalDate=desiredDate, 「翌日」→arrivalDate=発日+1日
時間: 「8時」「AM8」→"8:00", 「朝イチ」→"午前中", 「午後」→"午後", 「夕方」→"夕方以降", 時間帯は開始時刻を採用
金額: 2〜3桁のみ→千円単位（15→15000, 20→20000, 100→100000）, 「¥20,000」→20000, 「要相談」「応談」→"要相談"
荷種: 「宅配」「ヤマト」「佐川」「Amazon」→"宅配", 「弁当」「食品」→"食品", 「医薬」「医療」→"医療機器・医薬品", 「引越」→movingJob="引っ越し案件"
title自動生成: 「{departureArea}→{arrivalArea} {cargoType} {vehicleType}」（荷種不明は省略）
情報がないフィールドは空文字にすること。テキストに明記されていない情報を推測して入れないこと。${chatFewShotSection}`,
            },
