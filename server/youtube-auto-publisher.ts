...
export async function generateVideoScript(topic: string): Promise<{ title: string; script: string; description: string; tags: string[] }> {
  const response = await getOpenAI().chat.completions.create({
    ...
    messages: [
      {
        role: "system",
        content: `あなたは軽貨物配送業界の専門YouTubeチャンネル「軽貨物マッチ公式」の台本作成者です。
視聴者は軽貨物ドライバー、個人事業主、荷主企業の物流担当者です。
軽貨物マッチは軽貨物案件マッチングプラットフォームで、荷主と軽貨物ドライバーを効率的にマッチングするサービスです。
サイトURL: ${SITE_URL}
...
以下のJSON形式で出力してください:
{
  ...
  "script": "ナレーション台本（3000〜4000文字。以下の構成で:\n1. フック（最初の30秒で視聴者を引きつける問いかけや衝撃的な事実）\n2. チャンネル紹介（軽貨物マッチ公式チャンネルです、と簡潔に）\n3. 本題（3〜5つのポイントに分けて詳しく解説。具体例や数字を交えて）\n4. 実践的なアドバイス（すぐに使える具体的なノウハウ）\n5. まとめ（要点を簡潔に振り返り）\n6. CTA（チャンネル登録・概要欄の軽貨物マッチリンクへの誘導）\n自然で落ち着いた話し言葉で。「では」「さて」「ここで重要なのが」など接続詞を使って流れを作る。「軽貨物マッチ」への誘導は本題の中で2〜3回自然に含める。段落間には間を置くため改行を入れる）",
  "description": "YouTube概要欄のテキスト（500〜800文字。動画の要約、タイムスタンプ（00:00形式で章立て）、関連キーワード、軽貨物マッチへのリンク・CTA含む）",
